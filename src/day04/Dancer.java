...
// 클래스는 객체를 만들어내는 설계도이다.
// 라이브러리 클래스 : 설계도의 역할만 함 실행은 안함. - main 이 업다.
public class Dancer {
    // 클래스에서는 객체의 속성과 기능을 정의한다.

    // 객체의 속성 : 객체를 표현하는 데이터
    // => 필드 (field)
    String dancerName;  // 댄서이름 객체의 속성인 field
    String crewName;    // 팀 이름
    String genre;       // 장르
    int danceLevel;     // 0: 초보, 1: 아마추어, 2: 프로

    // 객체의 기능 : 객체가 할 수 있는 일, 행위
    // -> 메서드 (method), static 을 붙이지 않기.
    ...
    // 생성자는 여러개 만들 수 있음 : 생성자 오버로딩 (적재하다, 쌓다)
    // 규칙 : 파라미터가 달라야 함.
    Dancer(String dName){
        dancerName = dName;
        crewName = "도시의춤꾼들";
